...
public class UserSessionBean implements UserSessionBeanLocal {
    ...
    public void Registration(TblUser user) {
        ...
        //Get Last Added User's ID
        Collection<TblUser> lastuser = em.createNamedQuery("TblUser.findByIdLast").setMaxResults(1).getResultList();
        TblUser usr = new TblUser();
        Collection<Integer> userId = new ArrayList<Integer>();
        for (TblUser key : lastuser) {
            usr = em.find(TblUser.class, key.getUserId());
            userId.add(key.getUserId());
            System.out.println(key.getUserId());
        }

        TblUser userr = em.find(TblUser.class, usr.getUserId());
        Collection<TblGroup> groups = userr.getTblGroupCollection();

        for (Integer us : userId) {
            TblGroup group = em.find(TblGroup.class, 2);
            if (!groups.contains(group)) {
                Collection<TblUser> userss = group.getTblUserCollection();
                userss.add(userr);
                groups.add(group);

                group.setTblUserCollection(userss);
                userr.setTblGroupCollection(groups);
